...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  const row = await db.select({ appleSub: users.appleSub }).from(users).where(eq(users.id, auth.user.id)).get();
  return Response.json({ user: auth.user, hasAppleLinked: !!row?.appleSub });
}
    ...
    img.onload = () => {
      ...
      canvas.toBlob(
        (blob) => {
          ...
          const optimized = new File([blob], "photo.webp", { type: "image/webp" });
          resolve(optimized);
        },
        "image/webp",
        QUALITY,
...
export default function MyPage() {
  const { user, hasAppleLinked } = useLoaderData<typeof loader>();
  const actionData = useActionData<ActionData>();
  const unlinkFetcher = useFetcher<{ success?: boolean }>();
  const displayUser = actionData?.user ?? user;
  const isAppleLinked = unlinkFetcher.data?.success === true ? false : hasAppleLinked;
    ...
    <div className="mypage-container">
      <div className="profile-header">
        <div className="profile-photo-wrapper">
           <img
            src={displayUser.profilePhotoUrl || "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 120 120'%3E%3Crect width='120' height='120' rx='60' fill='%232a2a2a'/%3E%3Ccircle cx='60' cy='46' r='20' fill='%23555'/%3E%3Cpath d='M20 110c0-22 18-40 40-40s40 18 40 40' fill='%23555'/%3E%3C/svg%3E"}
            alt="Profile"
          ...
          <div className="profile-badges">
            ...
            {isAppleLinked ? (
              <div className="apple-linked-badge">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" width="10" height="13" fill="currentColor" aria-hidden="true">
                  <path d="M318.7 268.7c-.2-36.7 16.4-64.4 50-84.8-18.8-26.9-47.2-41.7-84.7-44.6-35.5-2.8-74.3 20.7-88.5 20.7-15 0-49.4-19.7-76.4-19.7C63.3 141.2 4 184.8 4 273.5c0 26.2 4.8 53.3 14.4 81.2 12.8 36.7 59 126.7 107.2 125.2 25.2-.6 43-17.9 75.8-17.9 31.8 0 48.3 17.9 76.4 17.9 48.6-.7 90.4-82.5 102.6-119.3-65.2-30.7-61.7-90-61.7-91.9zm-56.6-164.2c27.3-32.4 24.8-61.9 24-72.5-24.1 1.4-52 16.4-67.9 34.9-17.5 19.8-27.8 44.3-25.6 71.9 26.1 2 49.9-11.4 69.5-34.3z" />
                </svg>
                Apple 연결됨
                <unlinkFetcher.Form method="post" action="/api/auth/unlink-apple">
                  <button type="submit" className="apple-unlink-btn" aria-label="Apple 연결 해제">
                    ✕
                  </button>
                </unlinkFetcher.Form>
              </div>
              ...
              <a href="/api/auth/apple?link=true" className="apple-link-btn">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" width="11" height="14" fill="currentColor" aria-hidden="true">
                  <path d="M318.7 268.7c-.2-36.7 16.4-64.4 50-84.8-18.8-26.9-47.2-41.7-84.7-44.6-35.5-2.8-74.3 20.7-88.5 20.7-15 0-49.4-19.7-76.4-19.7C63.3 141.2 4 184.8 4 273.5c0 26.2 4.8 53.3 14.4 81.2 12.8 36.7 59 126.7 107.2 125.2 25.2-.6 43-17.9 75.8-17.9 31.8 0 48.3 17.9 76.4 17.9 48.6-.7 90.4-82.5 102.6-119.3-65.2-30.7-61.7-90-61.7-91.9zm-56.6-164.2c27.3-32.4 24.8-61.9 24-72.5-24.1 1.4-52 16.4-67.9 34.9-17.5 19.8-27.8 44.3-25.6 71.9 26.1 2 49.9-11.4 69.5-34.3z" />
                </svg>
                Apple 계정 연결하기
              </a>
